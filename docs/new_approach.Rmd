...
chile_catches <- read.csv(here::here("raw_data","chile_landings_with_GFW.csv"), stringsAsFactors = F, na.strings = "null") %>% 
  ...
  group_by(trip_id, year, departure, arrival, zone, length, capacity, power, aux_power, trip_days, distance_traveled, max_distance_from_shore) %>% 
...
```{r}
gfw_data <- readRDS("chilean_fleet_w_gfw.rds")
```
...
candidate_variables <- chile_catches_full %>% 
  select(-c(landings, mmsi, trip_id, zone)) %>% 
  colnames()
